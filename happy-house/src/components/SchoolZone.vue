      ...
      <div class="col-12 mb-4">
        <div class="card border-light shadow-sm ">
          <div class="card-body">
            ...
            <!-- searchBar end -->
            <p>{{ latlngTestMsg }}</p>
            <div v-if="totalSchoolCnt">
              <strong>{{ searchDong }}</strong
              >의 학교 검색 결과는 총 <strong>{{ totalSchoolCnt }}</strong
              >건 입니다.
              <div>
                결과를어떻게띄울까 test 검색결과
...
export default {
  ...
  mounted() {
    ...
    this.$store.commit('SET_CUR_PAGE', 'schoolZone');
      ...
      http
        ...
        .then(({ data }) => {
          ...
          if (data.result == 'login') {
            ...
            this.$swal({ icon: 'success', title: '학군 정보 검색에 성공했습니다!', showConfirmButton: false, timer: 1500 });
        ...
        .catch((error) => {
          ...
          if (error.response.status == '404') {
            this.$swal({ icon: 'error', title: '검색에 실패했습니다', text: '검색어를 확인해주세요.', footer: '서울특별시 내의 정보만을 제공하고 있습니다.' });
          } else {
